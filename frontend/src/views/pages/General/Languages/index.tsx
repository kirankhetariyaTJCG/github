...
const languages = [
  ...
  { name: '香港粵語', language_code: 'zh-hk', checked: true, isDefault: false }
]
  ...
  return (
    <Card sx={{ width: '100%', height: '100%' }}>
      <Box sx={{ p: 4, borderBottom: theme => `1px solid ${theme.palette.divider}` }}>
        <Typography sx={{ fontSize: '1.125rem', fontWeight: 600 }}>Supported Languages</Typography>
      </Box>
      <Box sx={{ p: 6 }}>
        <Grid container columnSpacing={4} sx={{ width: '100%', m: 0, justifyContent: 'center' }}>
          {Array.isArray(arr) &&
            ...
            arr?.map((item: any, index: number) => {
              return (
                <Grid key={index} item xs={12} sm={6} sx={{ pl: index % 2 === 0 ? '0 !important' : '1rem !important' }}>
                  <Box
                    sx={{
                      px: 4,
                      py: 2,
