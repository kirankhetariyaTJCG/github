...
// MUI Imports
import { Box, Dialog, DialogTitle, DialogContent, DialogActions, LoadingButton, TextField, IconButton, Typography, FormHelperText } from '@/Helper/MUIImports'
...
const AddEditArticle = (props: ModelProps & { handleSave: (data: any[]) => void, articles: any[] }) => {
  // Props
  const { open, setOpen, row, handleSave, articles } = props
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: EditorCommonTypes) => {
      ...
      Object?.keys(row)?.length > 0
        ? handleSave(articles?.map(item => item?._id === row?._id ? { ...data } : item))
        : handleSave([...articles, { ...data }])
    ...
    <Dialog open={open} maxWidth='md'>
      ...
      >
        <Typography sx={{ fontSize: '1.2rem', fontWeight: 600 }}>
          {Object?.keys(row)?.length > 0 ? 'Edit' : 'Add'} Article
        </Typography>
          ...
          {AppUtils.checkValue(formik.values.imgSrc) ? (
            ...
            <>
              ...
              <Box
                ...
                fontSize={50}
              />
              <Typography sx={{ mt: 2, fontWeight: 600, fontSize: '1.2rem' }}>
