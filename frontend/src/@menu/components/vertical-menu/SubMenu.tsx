...
import { LogFirebase } from '@/Helper/Firebase/firebase'
...
const SubMenu: ForwardRefRenderFunction<HTMLLIElement, SubMenuProps> = (props, ref) => {
  ...
  useEffect(() => {
    ...
    if (confirmUrlInChildren(children, pathname)) {
      RESTAURANT_DATA.currentpath = pathname
      LogFirebase("pathname", pathname)
      setActive(true)
          ...
          >
            {renderExpandIcon ? (
              ...
              <StyledVerticalNavExpandIcon open={isSubMenuOpen} transitionDuration={transitionDuration}>
                <Icon icon={'icon-park-outline:right-c'} style={{ color: '#fff' }} />
